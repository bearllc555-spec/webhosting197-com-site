...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!res.ok) {
      return new Response(JSON.stringify({ ok: false, error: 'twilio_error', status: res.status, twilio_message: j.message || text.substring(0, 200) }), { status: 502, headers });
    }
    if (j.status !== 'approved') {
      return new Response(JSON.stringify({ ok: false, error: 'invalid_code', twilio_status: j.status }), { status: 400, headers });
    }

    return new Response(JSON.stringify({ ok: true }), { status: 200, headers });
  } catch (err) {
    return new Response(JSON.stringify({ ok: false, error: 'unhandled', message: String(err && err.message || err) }), { status: 500, headers });
  }
}
